...
- (Annotation *)getAnnotation:(NSString *)filePath withPage:(NSNumber *)page
{
    ...
    else
    {
        ...
        NSArray *array = [objects sortedArrayUsingDescriptors:@[sort]];
        annotation = (Annotation *)[array lastObject];
    }
    request= nil;
    return annotation;
}

- (NSArray <Annotation *>*)getAnnotationArray:(NSString *)filePath withPage:(NSNumber *)page
{
    NSManagedObjectContext *context = [self managedObjectContext];
    NSEntityDescription *entityDesc = [NSEntityDescription entityForName:annotationEntity inManagedObjectContext:context];
    NSFetchRequest *request = [[NSFetchRequest alloc] init];
    ...
    {
        NSSortDescriptor *sort = [NSSortDescriptor sortDescriptorWithKey:@"date" ascending:YES];
        array = [objects sortedArrayUsingDescriptors:@[sort]];
    }
    request= nil;
    return array;
}
...
- (void)deleteAllData{
    NSFetchRequest *request = [NSFetchRequest fetchRequestWithEntityName:annotationEntity];
    NSError *error;
...
- (void)deleteAnnotation:(NSString *)filePath withPage:(NSNumber *)page isLast:(BOOL)isLast{
    
    NSArray <Annotation *>*array = [self getAnnotationArray:filePath withPage:page];
    
    for (NSInteger i = array.count - 1; i >= 0; i--) {
        [self.managedObjectContext deleteObject:array[i]];
        if (isLast) break;
    }
